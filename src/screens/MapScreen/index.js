...
import { Colors, AppStyles, Metrics } from "../../components/themes";
...
class MapScreen extends Component {
  ...
  state = {
    ...
    transition: {},
    //markers: this.props.Markerss.AllMarkers,
    region: {
  ...
  componentDidMount() {
    ...
    this.animation.addListener(({ value }) => {
      ...
      if (index >= this.props.Markerss.AllMarkers.length) {
        index = this.props.Markerss.AllMarkers.length - 1;
      }
      if (index <= 0) {
        index = 0;
      }
      ...
      this.regionTimeout = setTimeout(() => {
        if (this.index !== index) {
          this.index = index;
          const { coordinate } = this.props.Markerss.AllMarkers[index];
    ...
    this.setState(
      {
        openIndex: index,
        render: true,
        transition: this.props.Markerss.AllMarkers[index],
      },
      () => {
        this.images[
          index
  ...
  render() {
    const interpolations = this.props.Markerss.AllMarkers.map((marker, index) => {
      const inputRange = [
        (index - 1) * CARD_WIDTH,
        index * CARD_WIDTH,
   ...
   const markers = this.props.Markerss.AllMarkers
    {
      return this.props.Markerss.Loading == true ? <ActivityIndicator color='black' size='large' /> :
        <View style={styles.container}>
          ...
          <Icon
            ...
            styleIcon={[style.Side, AppStyles.shadow]}
            onPress={() => {
...
const mapStateToProps = state => {
  return {
    Markerss: state.Markers,
    Categoriess: state.Categories
  }
}
export default connect(mapStateToProps, { GetAllMarkers })(MapScreen);
